...
export async function createMouton(m: Omit<Mouton, 'id'>): Promise<number> {
  const { lastId } = await run(
    `INSERT INTO moutons
    (identification_number, name, photo, species, poultry_kind, race, sex, birth_date, acquisition_date, weight_kg, estimated_age_months, color, origin, purchase_price, status, observations)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      m.identification_number, m.name || null, m.photo || null, m.species, m.poultry_kind || null, m.race || null, m.sex,
      m.birth_date || null, m.acquisition_date || null, m.weight_kg ?? null, m.estimated_age_months ?? null, m.color || null,
...
export async function updateMouton(id: number, m: Partial<Mouton>): Promise<void> {
  ...
  await run(
    `UPDATE moutons SET identification_number=?, name=?, photo=?, species=?, poultry_kind=?, race=?, sex=?, birth_date=?, acquisition_date=?, weight_kg=?,
    ...
    [
      ...
      m.poultry_kind ?? base.poultry_kind ?? null,
      m.race ?? base.race ?? null,
      m.sex ?? base.sex,
      m.birth_date ?? base.birth_date ?? null,
      m.acquisition_date ?? base.acquisition_date ?? null,
      m.weight_kg ?? base.weight_kg ?? null,
